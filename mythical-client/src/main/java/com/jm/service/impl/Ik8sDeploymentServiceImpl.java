...
public class Ik8sDeploymentServiceImpl implements IK8sDeploymentService {
    ...
    @Override
    public LogWatch logwatch(String namespace, String deploymentName, OutputStream outputStream) {
        try {
            return k8sClientConfig.getClient().apps().deployments().inNamespace(namespace).withName(deploymentName).watchLog(outputStream);
        } catch (Exception e) {
            throw new RuntimeException(e.getMessage());
        }
    }
